...
public class SelectionTrajetServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        if (session == null || session.getAttribute("user") == null) {
            // Utilisateur non connecté -> rediriger vers login avec l'URL de retour
            String voyageId = request.getParameter("voyageId");
            String redirectUrl = request.getContextPath() + "/selection";
            if (voyageId != null && !voyageId.trim().isEmpty()) {
                redirectUrl += "?voyageId=" + voyageId;
            }
        ...
        // Vérifier si c'est un admin -> rediriger vers dashboard
