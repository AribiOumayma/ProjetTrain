...
public class MonCompteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) 
        ...
        // Load reservations for non-admin users
        if (!"admin".equals(user.getRole())) {
            // Fetch purchased reservations
            request.setAttribute("reservations", reservationDAO.findByUser(user));
            // Fetch used reservations for history
            request.setAttribute("usedReservations", reservationDAO.findByUserAndEtat(user, "utilisé"));
        }
